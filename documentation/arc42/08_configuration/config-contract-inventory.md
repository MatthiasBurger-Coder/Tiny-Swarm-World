...
| `TSW_LXC_DOCKER_REGISTRY_MIRROR` | unset; operator supplied | Optional | explicitly reviewed external Docker registry or Nexus proxy URL reachable from managed nodes; also used as the internal Tiny Swarm World Nexus Docker proxy upstream | `install.sh`, `composition.py`, docs |
...
| `TSW_INFISICAL_POSTGRES_IMAGE` | `postgres:14.23-alpine3.23` | image reference | Infisical compose, composition, docs |
| `TSW_INFISICAL_REDIS_IMAGE` | `redis:7.4.9-alpine3.21` | image reference | Infisical compose, composition, docs |
