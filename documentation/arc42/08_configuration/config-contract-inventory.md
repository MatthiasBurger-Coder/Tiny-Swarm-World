...
- `src/tiny_swarm_world/installer.py`: installer-local override path, required
  secret bootstrap derived from the secret manifest, default Traefik secret
  names, and setup invocation flags.
- `src/tiny_swarm_world/infrastructure/composition_configuration.py`:
  operator configuration keys, defaults, secret lookup, and typed validation
  helpers used by composition wiring. The public facade remains
...
|---|---|---|---|---|
| `TSW_PORTAINER_STACK_REQUEST_TIMEOUT_SECONDS` | repository default constant | Optional | positive integer seconds | `composition_configuration.py`, installation docs |
| `TSW_PORTAINER_USERNAME` | `admin` | Optional | identifier | `composition_configuration.py`, docs |
| `TSW_SONARQUBE_ADMIN_USERNAME` | `admin` | Optional | identifier | `composition_configuration.py` |
| `TSW_JENKINS_ADMIN_USERNAME` | `admin` | Optional | identifier | `composition_configuration.py` |
| `TSW_NEXUS_ADMIN_USERNAME` | `admin` | Optional | identifier | `composition_configuration.py`, docs |
| `TSW_SEED_INFISICAL_ITEMS` | `0` in installer | Optional | boolean flag as `0`/`1` | `installer.py`, `composition_configuration.py` |
| `TSW_LXC_DOCKER_REGISTRY_MIRROR` | unset; operator supplied | Optional | explicitly reviewed external Docker registry or Nexus proxy URL reachable from managed nodes; also used as the internal Tiny Swarm World Nexus Docker proxy upstream | `install.sh`, `composition_configuration.py`, docs |
| `TSW_PULSAR_ADMIN_URL` | unset | Optional | URL | configuration contract, docs |
| `TSW_PULSAR_PUBLIC_ADMIN_URL` | `http://localhost:14080` for direct diagnostic checks; `https://pulsar-api.tsw.local` is preferred routing | Optional | URL | configuration contract, browser/live tests, docs |
| `TSW_PULSAR_TOKEN_SECRET_KEY` | catalog-derived | Required | secret value | configuration contract, secret manifest, Pulsar compose |
| `TSW_PULSAR_ADMIN_TOKEN` | catalog-derived | Required | secret value | configuration contract, secret manifest, Pulsar compose, browser/live tests |
| `TSW_PULSAR_MANAGER_ADMIN_PASSWORD` | catalog-derived | Required | secret value | configuration contract, secret manifest, Pulsar Manager compose |
| `TSW_LXC_PROXY_LISTEN_ADDRESS` | implementation default | Optional | listen address | `composition_configuration.py` |
| `TSW_SWARM_REGISTRY_ENDPOINT` | implementation default | Optional | registry endpoint | `composition_configuration.py` |
| `TSW_NEXUS_DOCKER_HUB_PROXY_REPOSITORY` | implementation default | Optional | repository name | `composition_configuration.py`, docs |
| `TSW_NEXUS_DOCKER_HUB_PROXY_PORT` | implementation default | Optional | positive integer port | `composition_configuration.py`, docs |
...
| `TSW_INFISICAL_ORGANIZATION` | implementation default | Optional | identifier | `composition_configuration.py` |
| `TSW_INFISICAL_ADMIN_FIRST_NAME` | `Tiny` | Optional | text | `composition_configuration.py`, compose |
| `TSW_INFISICAL_ADMIN_LAST_NAME` | `Admin` | Optional | text | `composition_configuration.py`, compose |
...
|---|---|---|---|
| `TSW_NEXUS_IMAGE` | `sonatype/nexus3:3.75.1` | image reference | Nexus compose, composition_configuration.py, docs |
| `TSW_JENKINS_IMAGE` | local registry Jenkins image | image reference | Jenkins compose, composition_configuration.py, docs |
| `TSW_SERVICE_ACCESS_DASHBOARD_IMAGE` | local registry dashboard image | image reference | Service-access compose, composition_configuration.py |
| `TSW_SERVICE_ACCESS_NGINX_IMAGE` | local registry nginx image | image reference | Service-access compose, composition_configuration.py |
| `TSW_INFISICAL_IMAGE` | `infisical/infisical:v0.159.1` | image reference | Infisical compose, composition_configuration.py, docs |
| `TSW_INFISICAL_POSTGRES_IMAGE` | `postgres:14.23-alpine3.23` | image reference | Infisical compose, composition_configuration.py, docs |
| `TSW_INFISICAL_REDIS_IMAGE` | `redis:7.4.9-alpine3.21` | image reference | Infisical compose, composition_configuration.py, docs |
...
|---|---|---|---|
| `TSW_INSTALL_ENV_FILE` | `.tiny-swarm-world/local/live-installation.env` | local path | Optional operator override file; authorized WSL2 live runs must point to a WSL-native `0600` file outside `/mnt/*`. |
...
| `TSW_BOOTSTRAP_STATE_DIR` | unset | local path | Supported alias whose `bootstrap-secrets.env` file is an explicit input and is never generated. |
...
- Pulsar standalone is currently unauthenticated for local development; the
  active contract intentionally has URL overrides but no Pulsar password.
