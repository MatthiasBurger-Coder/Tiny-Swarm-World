...
On WSL hosts, stale Windows `netsh interface portproxy` rules can occupy
`127.0.0.1:80` and `127.0.0.1:443` before the fresh install reaches platform
exposure. When the failed preflight reports `PORT-80` or `PORT-443` for
Traefik, clear only those stale Windows host forwarding rules from an
Administrator PowerShell before rerunning `./install.sh`:

[source,powershell]
----
Set-ExecutionPolicy -Scope Process Bypass
.\tools\live\clear-wsl-traefik-portproxy.ps1
----

To inspect without deleting rules:
