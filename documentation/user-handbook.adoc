...
| Pulsar | `/pulsar` or `http://localhost:9527/#/environments` | Pulsar Manager UI; use the `platform/pulsar-manager` Infisical item for the UI password.
| Pulsar Admin API | `/pulsar-admin-api` or `http://localhost:8087/admin/v2/clusters` | Token-authenticated Admin API for local standalone broker; use the `platform/pulsar` Infisical item for the bearer token.
