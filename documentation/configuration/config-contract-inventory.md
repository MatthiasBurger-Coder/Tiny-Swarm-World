...
| `TSW_LXC_DOCKER_REGISTRY_MIRROR` | auto-detected when possible | Optional | external Docker registry or Nexus proxy URL reachable from managed nodes; also used as the internal Tiny Swarm World Nexus Docker proxy upstream | `install.sh`, `composition.py`, docs |
