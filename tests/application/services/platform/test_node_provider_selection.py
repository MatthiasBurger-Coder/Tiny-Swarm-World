...
class TestNodeProviderSelectionService(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_configured_backend_candidate_order_is_passed_to_readiness_port(self):
        readiness = _ReadinessProbe(
            ProviderReadiness(
                provider=NodeProviderKind.LXC_NATIVE,
                status=ProviderReadinessStatus.READY,
    ...
    async def test_ambiguous_incus_and_lxd_blocks_with_remediation(self):
        ...
        self.assertEqual(selection.backend_selection.candidates, (ManagedLxcBackend.INCUS, ManagedLxcBackend.LXD))
    ...
    async def test_missing_backend_blocks_without_provider_fallback(self):
        ...
        self.assertEqual(selection.backend_selection.evidence["classification"], "backend_missing")
        self.assertEqual(selection.backend_selection.evidence["host_kind"], "wsl2")
    ...
    async def test_failed_backend_probe_preserves_candidate_and_failure_evidence(self):
        ...
        self.assertEqual(result.evidence["reason"], "provider_selection_blocked")
        self.assertEqual(result.evidence["readiness_status"], "timeout")
        self.assertEqual(result.evidence["backend_status"], "unsupported")
        self.assertEqual(result.evidence["backend_candidate_count"], "1")
        self.assertEqual(result.evidence["backend_candidates"], "incus")
        self.assertEqual(result.evidence["backend"], "incus")
        self.assertEqual(result.evidence["classification"], "timeout")
        self.assertEqual(result.evidence["classification_source"], "timeout")
        self.assertEqual(result.evidence["remediation_count"], "1")
    ...
    async def test_blocked_selection_prevents_node_lifecycle_calls(self):
        ...
        self.assertEqual(VerificationStatus.BLOCKED, result.status)
        self.assertEqual(result.evidence["reason"], "provider_selection_blocked")
        self.assertEqual(result.evidence["classification"], "backend_missing")
        self.assertEqual(result.evidence["host_kind"], "wsl2")
        self.assertEqual(result.evidence["source"], "unit")
    ...
    async def test_selected_provider_calls_node_lifecycle_port(self):
        ...
        self.assertEqual(len(lifecycle.ensure_calls), 1)
    ...
    async def test_selected_provider_calls_managed_reset_port(self):
        ...
        self.assertEqual(VerificationStatus.VERIFIED, result.status)
        self.assertEqual(len(teardown.reset_calls), 1)
    ...
    async def test_destroy_step_calls_managed_destroy_port(self):
        ...
        self.assertEqual(VerificationStatus.VERIFIED, result.status)
        self.assertEqual(step.verification_target_id, "platform:destroy:managed-nodes")
        self.assertEqual(len(teardown.destroy_calls), 1)
    ...
    async def test_reset_step_calls_managed_reset_port_with_verification_target(self):
        ...
        self.assertEqual(VerificationStatus.VERIFIED, result.status)
        self.assertEqual(step.verification_target_id, "platform:reset:managed-nodes")
        self.assertEqual(len(teardown.reset_calls), 1)
        (reset_nodes, selection), = teardown.reset_calls
        self.assertEqual(nodes, reset_nodes)
        self.assertEqual(ManagedLxcBackend.INCUS, selection.backend_selection.backend)
    ...
    async def test_missing_teardown_port_blocks_managed_destroy_before_adapter_call(self):
        ...
        self.assertEqual(VerificationStatus.BLOCKED, result.status)
        self.assertEqual(result.evidence["reason"], "managed_node_teardown_port_missing")
    ...
    async def test_backend_mismatch_blocks_managed_reset_before_adapter_call(self):
        ...
        self.assertEqual(VerificationStatus.BLOCKED, result.status)
        self.assertEqual(result.evidence["reason"], "reset_node_backend_mismatch")
        self.assertEqual(teardown.reset_calls, [])
        self.assertEqual(teardown.destroy_calls, [])
    ...
    async def test_reset_step_without_managed_nodes_blocks_before_readiness_probe(self):
        ...
        self.assertEqual(VerificationStatus.BLOCKED, result.status)
        self.assertEqual(result.evidence["reason"], "managed_nodes_missing")
...
class _ReadinessProbe(PortNodeProviderReadiness):
    ...
    async def provider_readiness(
        ...
        self.calls.append((provider, preferred_backend, backend_candidates))
