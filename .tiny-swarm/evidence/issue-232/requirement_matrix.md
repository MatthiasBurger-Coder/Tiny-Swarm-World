...
| REQ-020 | Successful artifact readiness produces redacted evidence identifying contracts, targets and check status. | acceptance / observability | result/evidence adapters | Per-target live evidence includes status, scope, state and remediation | Readiness evidence serialization/redaction tests | VERIFIED_LOCAL |
| REQ-021 | Static checks remain non-mutating; live checks are consent-gated and use bounded timeouts. | acceptance / resilience | preflight/readiness adapters | Static port path and bounded readiness requests; setup outer consent guard | Static no-mutation, timeout and consent tests | VERIFIED_LOCAL |
