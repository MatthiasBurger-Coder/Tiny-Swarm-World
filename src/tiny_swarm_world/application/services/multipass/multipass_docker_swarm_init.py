...
class MultipassDockerSwarmInit:
    ...
    async def run(self):
        ...
        result = await self.command_workflow.run_sync("command_multipass_docker_swarm_manager_init.yaml")
        self.logger.info(f"Initializing Docker Swarm on Manager: {result}")

        self.logger.info("Getting join-token for the worker")
        result = await self.command_workflow.run_sync("command_multipass_docker_swarm_manager_join_token.yaml")
        self.logger.info(f"Getting join-token for the worker: {result}")

        self.logger.info("Getting Manager IP")
        result = await self.command_workflow.run_sync("command_multipass_docker_swarm_manager_ip.yaml")
        ...
        result = await self.command_workflow.run_sync("command_multipass_docker_swarm_join_worker.yaml", self.parameter)
    ...
    def _setup_commands_init(self, config_file: str, parameter: Optional[Dict[ParameterType, str]]) -> Dict[
        ...
        command_list = self.command_workflow.build_command_list(config_file, parameter)
