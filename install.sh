...
INFISICAL_SECRET_ENV_FILE="${TSW_INFISICAL_SECRET_ENV_FILE:-.tiny-swarm/secrets/infisical.local.env}"
...
`TSW_INFISICAL_POSTGRES_PASSWORD`. Infisical values are mirrored to the ignored
.tiny-swarm/secrets/infisical.local.env file and exported for the setup process.
EOF
}
...
write_infisical_secret_file() {
  mkdir -p "$(dirname "$INFISICAL_SECRET_ENV_FILE")"
  {
    ...
    write_export TSW_INFISICAL_PASSWORD "$TSW_INFISICAL_PASSWORD"
    ...
    write_export INITIAL_BOOTSTRAP_ADMIN_PASSWORD "$TSW_INFISICAL_PASSWORD"
